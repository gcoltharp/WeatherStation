...
def main():
    ...
    configurations = getConfigurations()
    deviceradius_mm=106.29
    # RPi.GPIO Layout 
    GPIO.setmode(GPIO.BCM)

    sensorpin=int(configurations["sensorgpios"][0]["gpiosensor4"])
    GPIO.setup(sensorpin, GPIO.IN)
    GPIO.add_event_detect(sensorpin, GPIO.RISING, callback = interrupt, bouncetime = 5)  
    global imp_per_sec
    global windspeed_mph
    ctr = 56
    while ctr > 0:
        
        windspeed_mph = float(( imp_per_sec * (deviceradius_mm * 6.28))*0.00223694)
        # insert values to db
        sqlCommand = "INSERT INTO windspeeddata SET dateandtime='%s',windspeedmph='%s'" % (currentTime,windspeed_mph)
